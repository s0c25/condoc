<div>
  <div class="px-4 py-5 bg-white sm:p-6">
    <div class="grid grid-cols-6 gap-6">
      ...
      {{-- Malformaciones de Feto: --}}

      <div class="col-span-6 sm:col-span-6 lg:col-span-2 ">
        <label for="mfg" class="font-semibold text-gray-700 block pb-1">
          Malformaciones Gastrointestinales</label>
        <select wire:model="mfg" class="form-multiselect block w-full" multiple id="my-multiselect" 
        required name="mfg[]">
          <option value='' selected disabled required>Selecciona opción</option>
          @foreach ($mf1g as $malformacione5)
          <option required value={{ $malformacione5->id }}>{{ $malformacione5->name }}</option>
          @endforeach
